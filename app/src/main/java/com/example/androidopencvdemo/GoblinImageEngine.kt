...
class GoblinImageEngine(val activity: ComponentActivity, val surfaceView: GoblinSurfaceView) {
    ...
    var callbackBodyMetrics: ((FloatArray) -> Unit)? = null
...
//        Log.d(TAG, "bitmapOut: ${bitmapOut.width} x ${bitmapOut.height}")
            ...
            val metrics = imageProcessorWrapper.processWithMetrics(rotation, bitmapIn, bitmapOut)
            if (metrics.size == 4 && metrics[3] > 0f) {
                callbackBodyMetrics?.invoke(metrics)
            }
