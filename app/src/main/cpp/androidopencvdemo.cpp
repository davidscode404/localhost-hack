...
Java_com_example_androidopencvdemo_ImageProcessorWrapper_processWithMetrics(JNIEnv *env, jobject thisObj,
                                                                            jint rotation, jobject bmIn,
                                                                            jobject bmOut) {
    jclass thisClass = env->GetObjectClass(thisObj);
    jfieldID fieldHandle = env->GetFieldID(thisClass, "handle", "J");
    jlong handle = env->GetLongField(thisObj, fieldHandle);
    ImageProcessor * pImageProcessor = (ImageProcessor *)handle;
    ...
    env->SetFloatArrayRegion(result, 0, 4, pImageProcessor->lastMetrics.data());
    return result;
}
